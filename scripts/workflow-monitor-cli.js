    ...
    try {
      ...
      for (let i = 0; i < keys.length - 1; i++) {
        if (dangerousKeys.includes(keys[i])) {
          throw new Error(`Prototype pollution attempt blocked: "${keys[i]}" is not allowed in configuration keys`)
        }
        if (!current[keys[i]]) {
          current[keys[i]] = {}
        }
      ...
      if (dangerousKeys.includes(lastKey)) {
        throw new Error(`Prototype pollution attempt blocked: "${lastKey}" is not allowed in configuration keys`)
      }
