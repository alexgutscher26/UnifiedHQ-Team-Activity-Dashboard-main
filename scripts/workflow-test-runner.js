...
class WorkflowTestRunner {
    ...
    async runSyntaxValidation() {
        ...
        const result = await this.runScript('workflow-validate.js', ['validate', '--all', '--json'], { silent: true });
    ...
    async runRequirementsValidation() {
        ...
        const result = await this.runScript('workflow-requirements-validator.js', ['validate', '--json'], { silent: true });
        ...
        if (result.success) {
            try {
                ...
                console.log(`   Passed criteria: ${validation.summary.passedCriteria}/${validation.summary.totalCriteria}`);

                return validation.summary.passRate >= 80; // 80% pass rate threshold
            } catch (error) {
                console.error('❌ Failed to parse requirements validation results:', error.message);
                return false;
            }
        } else {
    ...
    async runConfigurationTests() {
        ...
        const result = await this.runScript('workflow-test.js', ['report', '--json'], { silent: true });
        ...
        if (result.success) {
            try {
                ...
                console.error('❌ Failed to parse configuration test results:', error.message);
                return false;
            }
        } else {
    ...
    async runLocalTests(workflowName = null) {
        ...
        try {
            ...
            this.testResults.localTests = { skipped: true, reason: 'act not installed' };
        ...
        if (workflowName) {
            ...
            const result = await this.runScript('workflow-test.js', ['test', workflowName, '--dry-run'], { silent: false });
            testResults.push({
            ...
            for (const workflow of keyWorkflows) {
                console.log(`\n🧪 Testing ${workflow}...`);
                const result = await this.runScript('workflow-test.js', ['test', workflow, '--dry-run'], { silent: true });
                testResults.push({
                    workflow,
                    success: result.success,
                    output: result.output,
        ...
        console.log(`   Passed: ${this.testResults.localTests.passedTests}/${this.testResults.localTests.totalTests}`);
    ...
    generateReport() {
        ...
        if (this.testResults.localTests?.skipped ||
            (this.testResults.localTests?.passedTests === this.testResults.localTests?.totalTests)) {
            report.summary.passedSteps++;
        }

        report.summary.overallSuccess = report.summary.passedSteps === report.summary.totalSteps;

        // Generate recommendations
        if (this.testResults.syntaxValidation?.summary?.invalid > 0) {
            report.recommendations.push('Fix workflow syntax errors before proceeding');
        }

        if (this.testResults.requirementsValidation?.summary?.passRate < 80) {
            report.recommendations.push('Improve workflow coverage to meet more requirements');
        }

        if (!this.testResults.configurationTests?.actInstalled) {
            report.recommendations.push('Install act tool for local workflow testing');
        }

        if (this.testResults.localTests?.passedTests < this.testResults.localTests?.totalTests) {
    ...
    printSummary(report) {
        ...
        // Syntax validation
        const syntaxStatus = this.testResults.syntaxValidation?.summary?.invalid === 0 ? '✅' : '❌';
        console.log(`   ${syntaxStatus} Syntax Validation`);

        // Requirements validation
        const reqsStatus = this.testResults.requirementsValidation?.summary?.passRate >= 80 ? '✅' : '❌';
        const reqsRate = this.testResults.requirementsValidation?.summary?.passRate || 0;
        console.log(`   ${reqsStatus} Requirements Validation (${reqsRate}%)`);

        // Configuration tests
        const configStatus = this.testResults.configurationTests?.summary?.invalid === 0 ? '✅' : '❌';
        ...
        if (this.testResults.localTests?.skipped) {
            localStatus = '⚠️ ';
        } else if (this.testResults.localTests?.passedTests < this.testResults.localTests?.totalTests) {
            localStatus = '❌';
        }
        console.log(`   ${localStatus} Local Tests ${this.testResults.localTests?.skipped ? '(skipped)' : ''}`);
    }
    ...
    async runAllTests(options = {}) {
        ...
        try {
            ...
            if (!options.skipLocal) {
                ...
                this.testResults.localTests = { skipped: true, reason: 'user requested skip' };
...
program
    ...
    .action(async () => {
        ...
        if (setupResult.success) {
            ...
            console.log('   3. Run full test suite: node scripts/workflow-test-runner.js test');
        } else {
