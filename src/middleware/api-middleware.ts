...
export const withPublicMiddleware = (
  handler: (req: NextRequest) => Promise<NextResponse>
) => withApiMiddleware(handler, MiddlewareConfigs.PUBLIC);

export const withStrictMiddleware = (
  handler: (req: NextRequest) => Promise<NextResponse>
) => withApiMiddleware(handler, MiddlewareConfigs.STRICT);

export const withInternalMiddleware = (
  handler: (req: NextRequest) => Promise<NextResponse>
...
export async function withCustomRateLimit(
  ...
  try {
    const { RateLimiters, getClientIdentifier } = await import(
      '@/lib/rate-limiter'
    );
