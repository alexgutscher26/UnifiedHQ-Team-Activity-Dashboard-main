...
export function AIInsightsContent() {
            ...
            const mockInsights: AIInsight[] = [
                {
                    ...
                    description: 'Your team shows highest commit activity between 10 AM - 12 PM and 2 PM - 4 PM. Consider scheduling important meetings outside these windows.',
                    confidence: 0.87,
                    impact: 'high',
                ...
                },
                {
                    ...
                    description: 'Pull requests are taking 40% longer to review than industry average. Consider implementing automated code quality checks.',
                    confidence: 0.92,
                    impact: 'high',
                ...
                },
                {
                    ...
                    description: 'Daily commit frequency has increased by 25% over the past month, indicating improved development velocity.',
                    confidence: 0.78,
                    impact: 'medium',
                    category: 'Trends',
                ...
                },
                {
                    ...
                    description: 'Large files and deep directory structures detected. Consider refactoring to improve build times and developer experience.',
                    confidence: 0.65,
                    impact: 'medium',
                ...
                },
                {
                    ...
                    description: 'Slack activity shows increased cross-functional collaboration, with 35% more messages between development and design teams.',
                    confidence: 0.81,
                    impact: 'medium',
                    category: 'Collaboration',
    ...
    const fetchMetrics = async () => {
        try {
            const response = await fetch(`/api/ai-insights/metrics?timeRange=${timeRange}`);
    ...
    const generateNewInsights = async () => {
        try {
            ...
            toast({
                ...
                description: error instanceof Error ? error.message : 'Failed to generate new insights',
                variant: 'destructive',
            });
        } finally {
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            case 'productivity': return <IconTrendingUp className="size-4" />;
            case 'collaboration': return <IconUsers className="size-4" />;
            case 'code_quality': return <IconGitCommit className="size-4" />;
            case 'trend': return <IconChartBar className="size-4" />;
            case 'recommendation': return <IconTarget className="size-4" />;
            default: return <IconBrain className="size-4" />;
        }
    };

    const getTrendIcon = (trend: string) => {
        switch (trend) {
            case 'up': return <IconArrowUp className="size-4 text-green-500" />;
            case 'down': return <IconArrowDown className="size-4 text-red-500" />;
            case 'stable': return <IconMinus className="size-4 text-gray-500" />;
            default: return <IconMinus className="size-4 text-gray-500" />;
        }
    };
        ...
        <div className="flex flex-1 flex-col gap-4 p-4">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Intelligent analysis of your team's productivity and collaboration patterns
                    </p>
                </div>
            ...
            {metrics && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground">
                                ...
                                {((metrics.commitFrequency.current - metrics.commitFrequency.previous) / metrics.commitFrequency.previous * 100).toFixed(1)}% from last period
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium">Code Review Time</CardTitle>
            ...
            <Tabs defaultValue="insights" className="space-y-4">
                ...
                <TabsContent value="productivity">
                    <div className="space-y-4">
                        {insights.filter(i => i.type === 'productivity').map(insight => (
                            <Card key={insight.id}>
                                <CardHeader>
                ...
                <TabsContent value="collaboration">
                    <div className="space-y-4">
                        {insights.filter(i => i.type === 'collaboration').map(insight => (
                            <Card key={insight.id}>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
