...
export function OfflineSyncNotifications() {
    ...
    const showSyncCompletedNotification = (result: SyncResult) => {
        ...
        if (failedActions === 0) {
            ...
            toast({
                ...
                duration: 5000,
                action: (
                    <ToastAction altText="View sync details" onClick={() => showSyncDetails(result)}>
                        View Details
                    </ToastAction>
                ),
            });
            ...
            toast({
                ...
                action: (
                    <ToastAction altText="View sync errors" onClick={() => showSyncErrors(result)}>
                        View Errors
                    </ToastAction>
                ),
            });
    ...
    const showSyncFailedNotification = (error: string) => {
        toast({
            ...
            duration: 0, // Don't auto-dismiss
            action: (
                <ToastAction altText="Retry synchronization" onClick={() => triggerSync()}>
                    Retry
                </ToastAction>
            ),
        });
    ...
    const showConflictDetectedNotification = (conflict: any) => {
        toast({
            ...
            duration: 0, // Don't auto-dismiss
            action: (
                <ToastAction altText="Resolve data conflict" onClick={() => openConflictResolution(conflict)}>
                    Resolve
                </ToastAction>
            ),
        });
    ...
    const handleNetworkRestored = () => {
        toast({
            ...
            duration: 5000,
            action: (
                <ToastAction altText="Sync now" onClick={() => triggerSync()}>
                    Sync Now
                </ToastAction>
            ),
        });
                ...
                <div className='absolute right-0 top-full mt-2 w-80 bg-white border rounded-lg shadow-lg z-50'>
                    ...
                    <div className='max-h-64 overflow-y-auto'>
                        {notifications.map(notification => (
                            <div
                                key={notification.id}
                                className={`p-3 border-b last:border-b-0 ${notification.dismissed ? 'opacity-50' : ''
                                    }`}
                            >
