...
export function MemoryLeakDashboard({
    ...
    const [selectedSeverity, setSelectedSeverity] = useState<LeakSeverity | 'all'>('all');
    ...
    const filteredReports = state.projectReport?.reports.filter(report => {
        ...
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <RefreshCw className={`h-4 w-4 mr-2 ${state.isScanning ? 'animate-spin' : ''}`} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                            {state.lastScanTime ? `Last scan: ${state.lastScanTime.toLocaleTimeString()}` : 'No scan yet'}
                        </p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {state.runtimeReport ? formatMemoryUsage(state.runtimeReport.memoryUsage.current) : 'N/A'}
                        </div>
                        <p className="text-xs text-muted-foreground">
                            Current heap usage
                        </p>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Fixable Issues</CardTitle>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedSeverity(e.target.value as LeakSeverity | 'all')}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                value={selectedType}
                                onChange={(e) => setSelectedType(e.target.value as LeakType | 'all')}
                                className="px-3 py-1 border rounded-md text-sm"
                            >
                                <option value="all">All</option>
                                <option value="missing-useeffect-cleanup">useEffect Cleanup</option>
                                <option value="uncleaned-event-listener">Event Listeners</option>
                        ...
                        <div className="text-sm text-muted-foreground">
                            Showing {filteredReports.length} of {state.projectReport?.totalLeaks || 0} issues
                        </div>
                    </div>
                    ...
                    <ScrollArea className="h-[600px]">
                        <div className="space-y-3">
                            {filteredReports.map((report) => (
                                <Card key={report.id} className={`border-l-4 ${SEVERITY_COLORS[report.severity]}`}>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {report.suggestedFix && (
                                                <div className="text-sm">
                                                    ...
                                                    <p className="text-muted-foreground mt-1">{report.suggestedFix}</p>
                                                </div>
                                            )}
                                ...
                                <CardContent className="space-y-4">
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span>{formatMemoryUsage(state.runtimeReport.memoryUsage.current)}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span>Peak Usage</span>
                                            <span>{formatMemoryUsage(state.runtimeReport.memoryUsage.peak)}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span>Trend</span>
                                            <Badge variant={
                                                state.runtimeReport.memoryUsage.trend === 'growing' ? 'destructive' :
                                                    state.runtimeReport.memoryUsage.trend === 'declining' ? 'default' : 'secondary'
                                            }>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <Badge variant={state.runtimeReport.activeResources.eventListeners > 0 ? 'destructive' : 'secondary'}>
                                            {state.runtimeReport.activeResources.eventListeners}
                                        </Badge>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span>Intervals</span>
                                        <Badge variant={state.runtimeReport.activeResources.intervals > 0 ? 'destructive' : 'secondary'}>
                                            {state.runtimeReport.activeResources.intervals}
                                        </Badge>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span>Timeouts</span>
                                        <Badge variant={state.runtimeReport.activeResources.timeouts > 0 ? 'destructive' : 'secondary'}>
                                            {state.runtimeReport.activeResources.timeouts}
                                        </Badge>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span>Subscriptions</span>
                                        <Badge variant={state.runtimeReport.activeResources.subscriptions > 0 ? 'destructive' : 'secondary'}>
                                            {state.runtimeReport.activeResources.subscriptions}
                                        </Badge>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span>Connections</span>
                                        <Badge variant={state.runtimeReport.activeResources.connections > 0 ? 'destructive' : 'secondary'}>
                        ...
                        <Card>
                            <CardContent className="flex items-center justify-center py-8">
                                <div className="text-center">
                                    ...
                                    <h3 className="text-lg font-medium">Runtime Monitoring Disabled</h3>
                                    <p className="text-muted-foreground mb-4">
                                        Start monitoring to see real-time memory usage and resource tracking
                                    </p>
                ...
                <TabsContent value="trends" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Memory leak detection trends over time</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {state.scanHistory.length > 0 ? (
                                <div className="space-y-3">
                                    {state.scanHistory.slice(-10).reverse().map((scan, index) => (
                ...
                <TabsContent value="files" className="space-y-4">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Memory leak issues grouped by file</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <ScrollArea className="h-[400px]">
                                    <div className="space-y-2">
                                        {state.projectReport.files.map((file) => {
                                            const fileReports = state.projectReport!.reports.filter(r => r.file === file);
                                            const criticalCount = fileReports.filter(r => r.severity === 'critical').length;
                                            const highCount = fileReports.filter(r => r.severity === 'high').length;

                                            return (
                                                <div key={file} className="flex items-center justify-between py-2 px-3 border rounded">
