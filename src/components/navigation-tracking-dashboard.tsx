...
export function NavigationTrackingDashboard() {
  ...
  const { clearPatterns, preloadCriticalData } = useNavigationTracking({
    autoTrack: false,
  });
                    ...
                    >
                      <div className='flex-1'>
                        ...
                        <div className='flex items-center gap-4 text-sm text-muted-foreground'>
                          ...
                          <span>
                            Last: {formatTimestamp(pattern.lastAccessed)}
                          </span>
                          {pattern.preloadTargets.length > 0 && (
                            <span>
                              {pattern.preloadTargets.length} preload targets
                            </span>
                          )}
                        </div>
                        {pattern.preloadTargets.length > 0 && (
          ...
          {preloaderStats && (
            <Card>
              ...
              <CardContent>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-6'>
                  ...
                  <div className='text-center p-4 border rounded'>
                    <p className='text-2xl font-bold text-purple-500'>
                      {Math.round(
                        (preloaderStats.performance?.successRate || 0) * 100
                      )}
                      %
                    </p>
                    <p className='text-sm text-muted-foreground'>
                ...
                {preloaderStats.queue?.items &&
                  preloaderStats.queue.items.length > 0 && (
                    <div className='space-y-3'>
                      ...
                      {preloaderStats.queue.items.length > 5 && (
                        <p className='text-sm text-muted-foreground text-center'>
                          ... and {preloaderStats.queue.items.length - 5} more
                          items
                        </p>
                      )}
                    </div>
                ...
                {preloaderStats.completed && (
                  <div className='mt-6 p-4 bg-muted rounded'>
                    ...
                    <div className='grid grid-cols-2 gap-4 text-sm'>
                      ...
                      <div>
                        Successful: {preloaderStats.completed.successful}
                      </div>
                      <div>Failed: {preloaderStats.completed.failed}</div>
                      <div>
                        Avg Time:{' '}
                        {Math.round(
                          preloaderStats.performance?.averageTime || 0
                        )}
                        ms
                      </div>
                    </div>
