...
async function getTeamActivity(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.log(`[Team Activity] Fetching activity for user: ${user.id}`);
    ...
    if (githubActivities.length === 0) {
      ...
      try {
        ...
        console.warn('[Team Activity] Database check failed:', dbError);
    ...
    console.log(`[Team Activity] Error for user ${user.id}:`, error);
...
export async function GET(request: NextRequest) {
  try {
    return await getTeamActivity(request);
  } catch (error) {
    console.error('[Team Activity API] Unhandled error:', error);
    return NextResponse.json(
      {
