...
export async function GET(request: NextRequest) {
  try {
    ...
    const insights = await generateInsightsFromActivities(
      activities,
      startDate,
      now
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { timeRange = '30d', forceRegenerate: _forceRegenerate = false } =
      body;
      ...
      return NextResponse.json(
        {
          ...
          message:
            'Connect your integrations and perform some activities to generate insights',
        },
        { status: 404 }
      );
    }
...
async function generateInsightsFromActivities(
  ...
  const firstHalf = activities.filter(
    a => new Date(a.timestamp) < midPoint
  ).length;
  const secondHalf = activities.filter(
    a => new Date(a.timestamp) >= midPoint
  ).length;
  ...
  if (secondHalf > firstHalf * 1.2) {
    insights.push({
      ...
      data: {
        firstHalf,
        secondHalf,
        increase: ((secondHalf - firstHalf) / firstHalf) * 100,
      },
      generatedAt: new Date().toISOString(),
      timeRange: {
        start: startDate.toISOString(),
  ...
  if (sourceBreakdown.github) {
    ...
    const commitCount = githubActivities.filter(
      a => a.metadata?.eventType === 'commit'
    ).length;
    const prCount = githubActivities.filter(
  ...
  if (Object.keys(sourceBreakdown).length === 1) {
    ...
    insights.push({
      ...
      data: {
        currentSources: Object.keys(sourceBreakdown),
        dominantSource: singleSource,
      },
      generatedAt: new Date().toISOString(),
      timeRange: {
        start: startDate.toISOString(),
  ...
  // Time-based insights
  const hourlyActivity = activities.reduce(
    (acc, activity) => {
