...
import { Fix, FixGenerationResult, CodeTransformation } from './memory-leak-fix-generator';
...
export class ConnectionCleanupFixGenerator {
    ...
    generateConnectionCleanupFix(leak: LeakDetectionResult): FixGenerationResult {
        try {
            ...
            if (!pattern) {
                return { success: false, error: 'Could not analyze connection pattern' };
            }

            // Generate appropriate cleanup transformation
            const transformation = this.generateConnectionCleanupTransformation(connectionNode, pattern);
    ...
    private analyzeConnectionPattern(node: ts.Node, leakType: string): ConnectionPattern | null {
    ...
    private analyzeConstructorPattern(node: ts.NewExpression, leakType: string): ConnectionPattern | null {
        ...
        const constructorArgs = node.arguments?.map(arg => this.getNodeText(arg)) || [];
    ...
    private analyzeSubscriptionPattern(node: ts.CallExpression, leakType: string): ConnectionPattern | null {
    ...
    private generateConnectionCleanupTransformation(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
        switch (pattern.connectionType) {
            ...
            default:
                throw new Error(`Unsupported connection type: ${pattern.connectionType}`);
        }
    }

    private generateEventSourceCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
    ...
    private generateWebSocketCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
        if (!pattern.isAssigned) {
            // Need to assign to variable first
    ...
    private generateSubscriptionCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
        if (!pattern.isAssigned) {
    ...
    private createEventSourceWithVariableAndCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
  ...
  // TODO: Call ${eventSourceVar}.close() when cleanup is needed`;
    ...
    private createWebSocketWithVariableAndCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
  ...
  // TODO: Call ${webSocketVar}.close() when cleanup is needed`;
    ...
    private createSubscriptionWithVariableAndCleanup(node: ts.Node, pattern: ConnectionPattern): CodeTransformation {
  ...
  // TODO: Call ${unsubscribeVar}() when cleanup is needed`;
    ...
    private addToExistingCleanup(node: ts.Node, cleanupCode: string): CodeTransformation {
        const containingFunction = this.findContainingFunction(node);
    ...
    private createCleanupFunction(node: ts.Node, cleanupCode: string): CodeTransformation {
        const containingFunction = this.findContainingFunction(node);
        if (!containingFunction) {
            throw new Error('No containing function found');
        }
    ...
    private wrapInUseEffectWithCleanup(node: ts.Node, cleanupCode: string, pattern: ConnectionPattern): CodeTransformation {
        const nodeText = this.getNodeText(node);
        const varName = pattern.variableName || this.getDefaultVariableName(pattern.connectionType);
    ...
    private addCleanupComment(node: ts.Node, cleanupCode: string): CodeTransformation {
        const nodeText = this.getNodeText(node);
    ...
    private addToExistingCleanupFunction(returnStatement: ts.ReturnStatement, cleanupCode: string): CodeTransformation {
    ...
    private hasJSXReturn(functionNode: ts.Node): boolean {
        ...
        const visit = (node: ts.Node) => {
            if (ts.isReturnStatement(node) && node.expression) {
                if (ts.isJsxElement(node.expression) || ts.isJsxFragment(node.expression) || ts.isJsxSelfClosingElement(node.expression)) {
                    hasJSX = true;
                }
            }
...
// Factory function
export function createConnectionCleanupFixGenerator(sourceCode: string, fileName: string): ConnectionCleanupFixGenerator {
