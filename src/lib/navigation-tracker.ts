...
class NavigationTracker {
  ...
  private async initializeServiceWorker(): Promise<void> {
    ...
    if ('serviceWorker' in navigator) {
      try {
        ...
        console.warn(
          '[NavigationTracker] Service worker not available:',
          error
        );
      }
    }
  ...
  async getNavigationPatterns(limit = 10): Promise<NavigationStats | null> {
    ...
    try {
      const response = await this.sendMessage('GET_NAVIGATION_PATTERNS', {
        limit,
      });
  ...
  async preloadCriticalData(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        '[NavigationTracker] Failed to preload critical data:',
        error
      );
      return false;
    }
  ...
  async getPreloaderStats(): Promise<any> {
    ...
    try {
      ...
      console.error(
        '[NavigationTracker] Failed to get preloader stats:',
        error
      );
      return null;
    }
  ...
  async clearPreloadQueue(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        '[NavigationTracker] Failed to clear preload queue:',
        error
      );
      return false;
    }
