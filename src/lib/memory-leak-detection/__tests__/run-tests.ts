...
class SimpleRuleTester {
    ...
    test(testCases: { valid: string[]; invalid: Array<{ code: string; expectedErrors: number }> }) {
        ...
        testCases.valid.forEach((code, index) => {
            try {
                ...
                const hasCleanup = code.includes('return () => {') &&
                ...
                if (hasCleanup || !this.hasMemoryLeakPattern(code)) {
                    ...
                    console.log(`  ✗ Valid case ${index + 1} failed - should not have memory leak`);
                    failed++;
                }
            } catch (error) {
        ...
        testCases.invalid.forEach((testCase, index) => {
            try {
                ...
                if (hasMemoryLeak) {
                    console.log(`  ✓ Invalid case ${index + 1} passed - detected memory leak`);
                    passed++;
                } else {
                    console.log(`  ✗ Invalid case ${index + 1} failed - should have detected memory leak`);
                    failed++;
                }
            } catch (error) {
    ...
    private hasMemoryLeakPattern(code: string): boolean {
        ...
        const hasInterval = code.includes('setInterval') && !code.includes('clearInterval');
        const hasTimeout = code.includes('setTimeout') && !code.includes('clearTimeout');
        const hasEventSource = code.includes('new EventSource') && !code.includes('.close()');
        const hasWebSocket = code.includes('new WebSocket') && !code.includes('.close()');
        const hasSubscription = code.includes('.subscribe(') && !code.includes('.unsubscribe()');

        return hasEventListener || hasInterval || hasTimeout || hasEventSource || hasWebSocket || hasSubscription;
    }
}
...
async function runTests() {
    ...
    const tester = new SimpleRuleTester('useeffect-cleanup', useEffectCleanupRule);
