        ...
        try {
            switch (leak.type) {
                case 'event-listener':
                    ...
                    break;
                case 'interval':
                    leak.intervals?.forEach((interval: NodeJS.Timeout) => clearInterval(interval));
                    break;
                case 'timeout':
                    leak.timeouts?.forEach((timeout: NodeJS.Timeout) => clearTimeout(timeout));
                    break;
                case 'subscription':
                    leak.subscriptions?.forEach((unsubscribe: () => void) => unsubscribe());
                    break;
                case 'memory':
...
export class MemoryLeakValidator {
    ...
    async validateDetectionAccuracy(
        ...
        for (const testCase of testCases) {
            ...
            try {
                ...
                const tp = testCase.expectedLeaks.filter(type => detectedSet.has(type)).length;
                ...
                const fn = testCase.expectedLeaks.filter(type => !detectedSet.has(type)).length;
    ...
    async validateFixEffectiveness(
        ...
        try {
            ...
            const effectiveness = originalLeakCount > 0 ? leaksFixed / originalLeakCount : 1;

            // Check for new issues introduced by fixes
            const originalLeakTypes = new Set(originalLeaks.map(l => `${l.type}:${l.line}`));
    ...
    async validateRuntimeDetection(
        ...
        const monitorInterval = setInterval(async () => {
            try {
                ...
                if (simulator.getActiveLeakCount() === 0 && runtimeReport.suspiciousPatterns.length > 0) {
        ...
        const detectionRate = simulatedLeaks.length > 0 ? detectedLeaks / simulatedLeaks.length : 0;
