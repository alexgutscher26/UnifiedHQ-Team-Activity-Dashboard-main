...
export class CodeAnalyzer {
  ...
  private analyzeNode(
    ...
    ts.forEachChild(node, child =>
      this.analyzeNode(child, leaks, cleanupActions)
    );
  }
  ...
  private analyzeCallback(
    ...
    potentialLeaks: Map<string, MemoryLeak>
  ): void {
    /**
  ...
  private generateSuggestions(
    ...
    if (leaks.length === 0) {
      ...
      if (cleanupActions.length === 0) {
        suggestions.push(
          '💡 Add a cleanup function that returns from useEffect'
        );
      }
    }
  ...
  private analyzeComponentDidMount(
    node: ts.Node,
    leaks: MemoryLeak[],
    cleanupActions: CleanupAction[]
  ): void {
 ...
 */
export function analyzeCodeForMemoryLeaks(
  code: string,
  fileName?: string
): MemoryLeakAnalysis {
...
export function generateMemoryLeakReport(analysis: MemoryLeakAnalysis): string {
  ...
  lines.push(
    `Status: ${analysis.hasMemoryLeaks ? '❌ Issues Found' : '✅ Clean'}`
  );
  lines.push('');
