    ...
    for (const file of files) {
        try {
            ...
            if (analysis.hasMemoryLeaks) {
                ...
                console.log(`✅ ${path.relative(process.cwd(), file)} - Clean (${analysis.score}/100)`);
            }
...
function findReactFiles(dir: string): string[] {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (/\.(tsx?|jsx?)$/.test(entry.name) && !entry.name.endsWith('.test.tsx') && !entry.name.endsWith('.test.ts')) {
                files.push(fullPath);
            }
        }
    }
