...
export const useEffectCleanupRule = ESLintUtils.RuleCreator(
    name => `https://github.com/your-org/eslint-plugin-memory-leak-detection/blob/main/docs/rules/${name}.md`
)({
    name: 'useeffect-cleanup',
    meta: {
        type: 'problem',
        docs: {
            description: 'Require cleanup functions in useEffect hooks that add event listeners, intervals, or subscriptions',
            recommended: 'error',
        },
        fixable: 'code',
        schema: [
        ...
        messages: {
            missingCleanup: 'useEffect hook with side effects is missing a cleanup function',
            missingIntervalCleanup: 'setInterval call in useEffect is missing clearInterval in cleanup',
            missingTimeoutCleanup: 'setTimeout call in useEffect is missing clearTimeout in cleanup',
            missingEventListenerCleanup: 'addEventListener call in useEffect is missing removeEventListener in cleanup',
            missingSubscriptionCleanup: 'Subscription in useEffect is missing unsubscribe in cleanup',
        },
    },
    defaultOptions: [
        {
    ...
    create(context, [options]) {
        ...
        function hasCleanupFunction(effectCallback: TSESTree.Function): boolean {
            ...
            return effectCallback.body.body.some(statement =>
        ...
        function generateCleanupCode(sideEffects: Array<{
            type: 'interval' | 'timeout' | 'eventListener' | 'subscription';
            node: TSESTree.Node;
        }>): string {
            ...
            sideEffects.forEach(effect => {
                switch (effect.type) {
                    ...
                    case 'timeout':
                        ...
                        break;
                    case 'eventListener':
                        if (effect.node.type === 'CallExpression' &&
                            effect.node.callee.type === 'MemberExpression') {
                            const objectCode = sourceCode.getText(effect.node.callee.object);
                            const eventType = effect.node.arguments[0] ? sourceCode.getText(effect.node.arguments[0]) : "'event'";
                            const listener = effect.node.arguments[1] ? sourceCode.getText(effect.node.arguments[1]) : 'listener';
                            cleanupStatements.push(`${objectCode}.removeEventListener(${eventType}, ${listener});`);
                        }
                        break;
                    case 'subscription':
                ...
                if (!hasCleanup) {
                    context.report({
                        ...
                        fix(fixer) {
                            ...
                            const lastStatement = effectCallback.body.body[effectCallback.body.body.length - 1];

                            if (lastStatement) {
                                return fixer.insertTextAfter(lastStatement, `\n  ${cleanupCode}`);
                            } else {
                                return fixer.insertTextAfter(effectCallback.body.body[0] || effectCallback.body, `\n  ${cleanupCode}`);
                            }
                        },
                    });
                }
...
export const eventListenerCleanupRule = ESLintUtils.RuleCreator(
    ...
    meta: {
        ...
        schema: [],
        messages: {
            missingRemoveEventListener: 'addEventListener call is missing corresponding removeEventListener',
        },
    },
    defaultOptions: [],
    create(context) {
            ...
            CallExpression(node: TSESTree.CallExpression) {
                if (
                    ...
                    if (node.callee.property.name === 'addEventListener') {
                        ...
                        const addKey = context.getSourceCode().getText(node).replace('removeEventListener', 'addEventListener');
...
export const intervalTimeoutCleanupRule = ESLintUtils.RuleCreator(
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Require clearInterval/clearTimeout for every setInterval/setTimeout call',
            recommended: 'error',
        },
        fixable: 'code',
        schema: [],
        messages: {
            missingClearInterval: 'setInterval call is missing corresponding clearInterval',
            missingClearTimeout: 'setTimeout call is missing corresponding clearTimeout',
        },
    },
    defaultOptions: [],
    create(context) {
...
export const subscriptionCleanupRule = ESLintUtils.RuleCreator(
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Require unsubscribe/close for subscriptions and connections',
            recommended: 'error',
        },
        fixable: 'code',
        schema: [],
