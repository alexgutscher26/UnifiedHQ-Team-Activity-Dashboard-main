...
export class SubscriptionLeakDetector {
    ...
    if (
      ...
      if (!hasCleanup) {
        ...
        this.results.push({
          ...
          description:
            'EventSource created without proper cleanup. This can cause memory leaks.',
          suggestedFix:
            'Add eventSource.close() in useEffect cleanup or component unmount',
    ...
    if (
      ...
      if (!hasCleanup) {
        ...
        this.results.push({
          ...
          column: position.character + 1,
          description:
            'WebSocket created without proper cleanup. This can cause memory leaks.',
          suggestedFix:
            'Add webSocket.close() in useEffect cleanup or component unmount',
          codeSnippet: this.getCodeSnippet(node),
        });
      }
    }
    ...
    if (
      ...
      if (!hasUnsubscribe) {
        ...
        this.results.push({
          ...
          column: position.character + 1,
          description:
            'Subscription created without unsubscribe. This can cause memory leaks.',
          suggestedFix:
            'Store the unsubscribe function and call it in useEffect cleanup',
          codeSnippet: this.getCodeSnippet(node),
        });
      }
    }
      ...
      if (
        ...
        if (hasSubscriptions && !hasCleanup) {
          ...
          this.results.push({
            ...
            column: position.character + 1,
            description:
              'useEffect contains subscriptions but no cleanup function. This can cause memory leaks.',
            suggestedFix:
              'Add return statement with cleanup function in useEffect',
            codeSnippet: this.getCodeSnippet(node),
          });
        }
      }
  ...
  private hasUnsubscribePattern(node: ts.Node): boolean {
    ...
    return (
      this.hasMethodCall(parent, 'unsubscribe') ||
      this.hasVariableAssignment(parent, node) ||
      this.hasCleanupReturn(parent)
    );
  }

  private hasSubscriptionsInEffect(
    effectFunction: ts.ArrowFunction | ts.FunctionExpression
  ): boolean {
  ...
  private hasVariableAssignment(
    parent: ts.Node,
    subscribeNode: ts.Node
  ): boolean {
...
export function createSubscriptionLeakDetector(
  sourceCode: string,
  fileName: string
...
export async function scanFileForSubscriptionLeaks(
  filePath: string
...
export async function scanFilesForSubscriptionLeaks(
  filePaths: string[]
): Promise<LeakDetectionResult[]> {
