...
const requireUseEffectCleanup = {
  meta: {
    ...
    docs: {
      description:
        'Require cleanup functions in useEffect hooks that create subscriptions or listeners',
    ...
    messages: {
      missingCleanup:
        'useEffect hook with {{type}} should return a cleanup function',
      missingCleanupGeneric:
        'useEffect hook should return a cleanup function for proper resource management',
    },
  },
  ...
  create(context) {
    ...
    function hasCleanupReturn(effectBody) {
      ...
      const returnStatements = effectBody.body.filter(
    ...
    function detectLeakPatterns(effectBody) {
      ...
      function walkNode(node) {
        ...
        if (
          ...
          patterns.push(
            node.callee.name === 'setInterval' ? 'interval' : 'timeout'
          );
        }
            ...
            if (patterns.length > 0 && !hasCleanupReturn(effectBody)) {
              ...
              context.report({
                ...
                fix(fixer) {
                  ...
                  if (cleanupCode) {
                    const lastStatement =
                      effectBody.body[effectBody.body.length - 1];
                    const insertPosition = lastStatement
                      ? lastStatement.range[1]
                      : effectBody.range[1] - 1;
  ...
  meta: {
    ...
    messages: {
      missingRemoveListener:
        'addEventListener call should have corresponding removeEventListener in cleanup',
      missingMediaQueryCleanup:
        'MediaQueryList.addListener should have corresponding removeListener in cleanup',
    },
  },
            ...
            context.report({
              ...
              fix(fixer) {
                ...
                if (useEffectNode && useEffectNode.arguments[0]) {
                  ...
                  if (effectBody) {
                    const cleanupCode = generateMediaQueryCleanupFix(
                      node,
                      target,
                      handler
                    );
...
const requireTimerCleanup = {
  meta: {
    ...
    docs: {
      description:
        'Require clearInterval/clearTimeout for timer functions in React components',
      category: 'Possible Errors',
      recommended: true,
    },
    fixable: 'code',
    ...
    messages: {
      missingClearInterval:
        'setInterval call should have corresponding clearInterval in cleanup',
      missingClearTimeout:
        'setTimeout call should have corresponding clearTimeout in cleanup',
    },
  },
  ...
  create(context) {
    ...
    function generateTimerCleanupFix(node, timerType, variableName) {
      const clearFunction =
        timerType === 'setInterval' ? 'clearInterval' : 'clearTimeout';
      const cleanupCode = `
    return () => {
      if (${variableName}) {
              ...
              context.report({
                ...
                fix(fixer) {
                  ...
                  if (useEffectNode && useEffectNode.arguments[0]) {
                    ...
                    if (effectBody) {
                      const cleanupCode = generateTimerCleanupFix(
                        node,
                        'setInterval',
                        variableName
                      );
...
const requireSubscriptionCleanup = {
  meta: {
    ...
    docs: {
      description:
        'Require cleanup for subscriptions, EventSource, and WebSocket connections',
      category: 'Possible Errors',
      recommended: true,
    },
    fixable: 'code',
    ...
    messages: {
      missingUnsubscribe:
        'Subscription should have corresponding unsubscribe in cleanup',
  ...
  create(context) {
    ...
    return {
                    ...
                    const cleanupMethod =
                      node.callee.property.name === 'subscribe'
                        ...
                        : node.callee.property.name === 'on'
                          ? 'off'
                          : 'removeListener';
                    const cleanupCode = generateSubscriptionCleanupFix(
                      node,
                      cleanupMethod,
                      variableName
                    );
      ...
      NewExpression(node) {
        ...
        if (node.callee && node.callee.name === 'EventSource') {
          if (isInReactComponent(node) && isInUseEffect(node)) {
            const variableName = findAssignedVariable(node);
        ...
        if (node.callee && node.callee.name === 'WebSocket') {
          if (isInReactComponent(node) && isInUseEffect(node)) {
            const variableName = findAssignedVariable(node);
