...
export class TimerCleanupFixGenerator {
    ...
    generateTimerCleanupFix(leak: LeakDetectionResult): FixGenerationResult {
        try {
            ...
            const transformation = this.generateTimerCleanupTransformation(timerNode, pattern);
    ...
    private analyzeTimerPattern(node: ts.CallExpression): TimerPattern | null {
        ...
        const callback = node.arguments[0] ? this.getNodeText(node.arguments[0]) : '';
    ...
    private generateTimerCleanupTransformation(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
    ...
    private createTimerWithVariableAndCleanup(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
  ...
  // TODO: Call ${pattern.clearFunction}(${timerVar}) when cleanup is needed`;
    ...
    private addToExistingUseEffectCleanup(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
        const containingFunction = this.findContainingFunction(node);
    ...
    private createUseEffectCleanup(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
        const containingFunction = this.findContainingFunction(node);
        if (!containingFunction) {
            throw new Error('No containing function found');
        }
    ...
    private wrapInUseEffectWithCleanup(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
        const timerCall = this.getNodeText(node);
        const timerVar = pattern.variableName || this.generateTimerVariableName(pattern);
    ...
    private addCleanupToCurrentScope(node: ts.CallExpression, pattern: TimerPattern): CodeTransformation {
    ...
    private generateTimerVariableName(pattern: TimerPattern): string {
        const base = pattern.timerFunction === 'setInterval' ? 'interval' : 'timeout';
        return `${base}Id`;
    }
    ...
    private isComplexCallback(callback: string): boolean {
        ...
        return callback.includes('{') || callback.includes('=>') || callback.length > 50;
    }
...
// Factory function
export function createTimerCleanupFixGenerator(sourceCode: string, fileName: string): TimerCleanupFixGenerator {
