...
export class EventListenerFixGenerator {
    ...
    generateEventListenerCleanupFix(leak: LeakDetectionResult): FixGenerationResult {
        try {
            ...
            if (!pattern) {
                return { success: false, error: 'Could not analyze event listener pattern' };
            }
    ...
    private analyzeEventListenerPattern(node: ts.CallExpression): EventListenerPattern | null {
    ...
    private determineCleanupStrategy(pattern: EventListenerPattern, context: EventListenerContext): CleanupStrategy {
    ...
    private generateRemoveEventListenerCode(pattern: EventListenerPattern): string {
    ...
    private addToExistingReturnFunction(returnStatement: ts.ReturnStatement, cleanupCode: string): CodeTransformation {
    ...
    private addNewReturnStatement(containingFunction: ts.Node, cleanupCode: string): CodeTransformation {
        const insertPosition = containingFunction.getEnd() - 1;
        const returnCode = `
    return () => {
      ${cleanupCode}
    ...
    private generateFixDescription(pattern: EventListenerPattern, strategy: CleanupStrategy): string {
    ...
    private calculateConfidence(pattern: EventListenerPattern, strategy: CleanupStrategy): number {
        let confidence = 0.9;
...
interface CleanupStrategy {
    type: 'add-to-existing-cleanup' | 'create-cleanup-function' | 'media-query-cleanup' | 'global-event-cleanup';
...
// Factory function
export function createEventListenerFixGenerator(sourceCode: string, fileName: string): EventListenerFixGenerator {
